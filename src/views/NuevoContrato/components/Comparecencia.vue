  ...
  <div class="row align-items-center">
    <div class="col-9">
      <div class="row">
        ...
        <div class="col-2">
          <material-checkbox id="editComparecencia" v-model="store.comparecenciaEdit"
            >Editar</material-checkbox
          >
        </div>
      </div>
      <div v-if="store2.propiedad" class="mb-3 mt-3 ms-3">
        <p
          ref="Comparecencia"
          :contenteditable="store.comparecenciaEdit"
        ...
        >
          ...
          <template v-if="store2.propiedad.propietario.type === 'persona'">
            ...
            {{ capitalize(store2.propiedad.propietario.apellidoMaterno) }}, cédula nacional de
            identidad n° {{ store2.propiedad.propietario.rut }}, nacionalidad
          ...
          <template v-else>
            empresa <strong>{{ capitalize(store2.propiedad.propietario.nombre) }} </strong>, rut
            ...
            {{ capitalize(store2.propiedad.propietario.representante.apellidoMaterno) }}, cédula
            nacional de identidad n° {{ store2.propiedad.propietario.representante.rut }},
            <template v-if="store2.propiedad.propietario.representante.tratamiento === 'Don.'"
              >domiciliado
            </template>
            <template v-else>domiciliada</template>
            en
          ...
          <template v-else>
            ...
            {{ capitalize(store2.arrendatario.representante.apellidoMaterno) }}, cédula nacional de
            identidad N° {{ store2.arrendatario.representante.rut }},
            {{ capitalize(store2.arrendatario.representante.nacionalidad) }},
            {{ capitalize(store2.arrendatario.representante.estadoCivil) }}, domiciliado en
          ...
          <template v-if="store2.codeudor">
            <template v-if="store2.codeudor.type === 'persona'">
              ...
              {{ store2.codeudor.rut }}, email: {{ store2.codeudor.email }}, celular
              {{ store2.codeudor.telefono }} , con domicilio en
              {{ store2.codeudor.direccion.calle }} {{ store2.codeudor.direccion.numero }}, comuna
              {{ store2.codeudor.direccion.ciudad }}, {{ store2.codeudor.direccion.region }}, en su
              calidad de Aval y Codeudor Solidario de la parte "arrendataria",
            </template>
            <template v-else-if="store2.codeudor.type === 'empresa'">
              y <strong>CODEUDOR</strong> la empresa
              <strong>{{ capitalize(store2.codeudor.nombre) }}</strong>
              ...
              {{ capitalize(store2.codeudor.representante.apellidoMaterno) }}, cédula nacional de
              identidad N° {{ store2.codeudor.representante.rut }},
              {{ capitalize(store2.codeudor.representante.nacionalidad) }},
              {{ capitalize(store2.codeudor.representante.estadoCivil) }}, domiciliado en
              {{ store2.codeudor.representante.direccion.calle }}
              {{ store2.codeudor.representante.direccion.numero }}, comuna
              {{ store2.codeudor.representante.direccion.ciudad }},
              {{ store2.codeudor.representante.direccion.region }}, celular
              {{ store2.codeudor.representante.telefono }}, email:
              {{ store2.codeudor.representante.email }}, en su calidad de Aval y Codeudor Solidario
