    ...
    <div class="col-9">
      <div class="row">
        ...
        <h5 id="Clausula13" class="col-8 card-title text-center">Clausula 13 ENTREGA Y RESTITUCION DEL
          INMUEBLE:</h5>
        <div class="col-2">
          <material-checkbox id="editClausula13" v-model="store.clausula13edit">Editar</material-checkbox>
        </div>
      </div>
      <div class="mb-3 mt-3 ms-3">
        <p ref="Clausula13" :contenteditable="store.clausula13edit" class="text-justify">
...
export default {
  ...
  watch: {
    'this.$refs.Clausula13': {
      handler(newVal) {
        if (newVal) {
          this.store.clausula13 = newVal.innerText;
        }
      },
      deep: true,
      immediate: true
  ...
  mounted() {
    if (this.$refs.Clausula13) {
      this.store.clausula13 = this.$refs.Clausula13.innerText;
    }
  }
}
</script>