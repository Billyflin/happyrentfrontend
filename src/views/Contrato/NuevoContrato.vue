        ...
        <div v-if="store2.propiedad.arrendado" class="row mt-4">
          <div class="col-12">
            <div class="alert alert-danger text-light text-center">
              La propiedad ya está arrendada
            </div>
          </div>
        </div>
        <div v-else>
          ...
          <div class="row mt-4">
            <div
              id="SeleccionArrendatario"
              :class="codeudor ? 'col-6' : 'col-10 col-xl-11 col-lg-11 flex-grow'"
            >
            ...
            <div v-else class="col">
              <material-button size="md" variant="success" @click="codeudor = true"
                >Agregar Codeudor</material-button
              >
            </div>
          </div>
          <!--          <div class="row mt-4">-->
          ...
          <router-link to="/EditorNuevoContrato">
            <material-button class="mt-4 mb-6" full-width size="lg" variant="success"
              >Crear Contrato</material-button
            >
          </router-link>
        </div>
      </div>
...
watch(
  () => [store2.codeudor?.id, store2.arrendatario?.id, store2.propiedad?.propietario.id],
  ([codeudorId, arrendatarioId, propietarioId]) => {
