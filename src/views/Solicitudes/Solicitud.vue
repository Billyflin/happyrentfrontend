  ...
  methods: {
    async emitData() {
      ...
      if (!this.v$.$error) {
        ...
        formData.append(
          'persona',
          new Blob([JSON.stringify(this.persona)], { type: 'application/json' })
        )
        this.files.forEach((file) => formData.append('files', file))
        try {
          await createSolicitud(this.token, formData)
            .then((it) => {
              console.log(it)
  ...
  setup(props) {
    ...
    onMounted(() => {
      ...
      if (
        props.carnet ||
        props.liquidaciones ||
        props.certificadoAFP ||
        props.carpetaTributaria ||
        props.certificadoDicom ||
        props.contratoTrabajo
      ) {
